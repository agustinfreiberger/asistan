package ar.edu.unicen.isistan.asistan.utils.markov;
...
public class HMM<O> {
    ...
    public HMM(int[][] observations, int[][] labels) {
        ...
        for (int i = 0; i < observations.length; i++) {
            ...
            numStates = Math.max(numStates, max(labels[i]) + 1);
            numSymbols = Math.max(numSymbols, max(observations[i]) + 1);
        }
    ...
    public HMM(O[][] observations, int[][] labels) {
        ...
        for (int i = 0; i < obs.length; i++) {
            numStates = Math.max(numStates, max(labels[i]) + 1);
            numSymbols = Math.max(numSymbols, max(obs[i]) + 1);
        }
     ...
     */
    private void unitize1(double[] x) {
        double n = norm1(x);
